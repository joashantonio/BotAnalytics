...
def _df_to_json(df: pd.DataFrame) -> str:
    """Serialize an OHLCV DataFrame to JSON: ISO-date index + OHLCV columns."""
...
def _download_ohlcv(ticker: str, from_date: str, to_date: str) -> pd.DataFrame:
    ...
    # drop trailing zero-volume bars (stale future/non-trading days)
...
def fetch_ohlcv(ticker: str, from_date: str, to_date: str) -> pd.DataFrame:
    """
    Fetch-once: returns stored OHLCV for (ticker, from, to) if present,
    else downloads from yfinance and persists the raw block to the DB.
    """
